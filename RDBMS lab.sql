...
/
--trigger before delete on dept table
CREATE OR REPLACE TRIGGER Dept_trigger_delete
...
BEGIN
	d:=:OLD.code;
	delete from teacher where dept=d;
	delete from batch where dept=d;
...
--trigger before delete on batch table
CREATE OR REPLACE TRIGGER batch_trigger_delete
...
BEGIN
	d:=:OLD.dept;
	e:=:OLD.semester;
	delete from course where dept=d and semester=e;
...
/
--trigger to delete TEACHER
CREATE OR REPLACE TRIGGER teacher_trigger_delete
...
BEGIN
	d:=:OLD.code;
	delete from routine where teacher=d;
...
/
CREATE OR REPLACE TRIGGER course_trigger_delete
...
BEGIN
	d:=:OLD.code;
	delete from routine where course=d;
...
/
--trigger before deleting lab
CREATE OR REPLACE TRIGGER lab_trigger_delete
...
BEGIN
	d:=:OLD.lab_no;
	delete from routine where lab=d;
...
/
CREATE OR REPLACE TRIGGER classroom_trigger_delete
...
BEGIN
	d:=:OLD.room_no;
	delete from routine where room=d;
...
create or replace procedure delete_routine(dep in varchar2,sem in number,crs in varchar2)
as 

begin 

delete from routine where course=crs;
